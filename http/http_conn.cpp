...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        char name[100] = {0};
        char password[100] = {0};
        // 假设报文格式为：user=XXX&password=YYY (注意原版可能使用的是 passwd，请根据前端一致性调整)
        // %99[^&] 表示最多读取 99 个非 '&' 的字符，防止超长字符串撑爆数组
        ...
        if (*(p + 1) == '3') // 注册请求
        {
            ...
            snprintf(sql_insert, sizeof(sql_insert), 
                     "INSERT INTO user(username, passwd) VALUES('%s', '%s')", name, password);
    ...
    if (*(p + 1) == '0')      target_file = "/register.html";
    else if (*(p + 1) == '1') target_file = "/log.html";
    else if (*(p + 1) == '5') target_file = "/picture.html";
    else if (*(p + 1) == '6') target_file = "/video.html";
    else if (*(p + 1) == '7') target_file = "/fans.html";
...
bool http_conn::write()
{
    ...
    while (1)
    {
        // 💥 核心：将内存中分散的两块数据（响应头 m_iv[0] 和 文件映射 m_iv[1]）一次性发给网卡
