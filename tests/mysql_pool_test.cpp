...
void task(int id) {
  MYSQL* conn = nullptr;
  {
    ...
    if (conn) {
      std::cout << "Thread " << id << " GOT connection! Doing work..."
                << std::endl;
      std::this_thread::sleep_for(
          std::chrono::milliseconds(500));  // 模拟查询耗时
    } else {
...
int main() {
  ...
  std::cout << "Pool initialized with " << pool->GetFreeConn()
            << " free connections." << std::endl;
  ...
  std::cout << "All requests handled. Remaining connections: "
